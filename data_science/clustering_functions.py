...
from data_science.data_science_functions import filter_and_clean_races, TOP_TIER_CLASSIFICATIONS, MAX_PROFILE_SCORE
...
def _sanitize_feature_name_for_path(feature_name: str) -> str:
    sanitized = "".join(
        character if character.isalnum() or character in {"_", "-"} else "_"
        for character in feature_name.strip()
    )
    return sanitized or "feature"


def _plot_continuous_feature_density(
    feature_name: str,
    ...
    for cluster_label in unique_labels:
        ...
        cluster_values = normalized_values[mask]
        ...
        ax.hist(
            ...
            linewidth=2,
            alpha=0.9,
            color=color_map.get(cluster_label, "#333333"),
            label=f"{cluster_label_to_name(cluster_label)} (n={cluster_values.size})",
        )
        plotted_any = True
    ...
    ax.set_title(f"1D density by cluster - {feature_name}")
    ax.grid(alpha=0.25)
...
def _plot_categorical_feature_density(
    feature_name: str,
    ...
    normalized_categories = np.array(
        ["missing" if value is None or value == "" else str(value) for value in categorical_values],
        dtype=object,
    )
    unique_categories = sorted(str(category) for category in np.unique(normalized_categories))
    unique_labels = sorted(int(label) for label in np.unique(labels))
    color_map = _cluster_label_color_map(labels)

    category_positions = np.arange(len(unique_categories), dtype=float)
    cluster_count = max(len(unique_labels), 1)
    bar_width = min(0.8 / cluster_count, 0.25)
    offset_center = (cluster_count - 1) / 2.0

    fig, ax = plt.subplots(figsize=(max(10, len(unique_categories) * 1.2), 6))
    for label_index, cluster_label in enumerate(unique_labels):
        cluster_mask = labels == cluster_label
        cluster_categories = normalized_categories[cluster_mask]
        if cluster_categories.size == 0:
            continue
        counts = np.array(
            [np.sum(cluster_categories == category) for category in unique_categories],
            dtype=float,
        )
        densities = counts / cluster_categories.size
        ax.bar(
            category_positions + (label_index - offset_center) * bar_width,
            densities,
            width=bar_width,
            color=color_map.get(cluster_label, "#333333"),
            alpha=0.85,
            label=f"{cluster_label_to_name(cluster_label)} (n={cluster_categories.size})",
        )

    ax.set_xticks(category_positions)
    ax.set_xticklabels(unique_categories, rotation=30, ha="right")
    ax.set_xlabel(feature_name)
    ax.set_ylabel("share within cluster")
    ax.set_ylim(0.0, 1.0)
    ax.set_title(f"1D category density by cluster - {feature_name}")
...
def one_d_density_visualisation(cut_k: int | None = None) -> list[str]:
    raw_df, df, selected_features, feature_space, dist_matrix = prepare_clustering_inputs()
    del raw_df, selected_features
    ...
    analyses_by_k = {
        int(analysis["n_clusters_requested"]): analysis
        for analysis in hierarchical_analyses
    }

    if cut_k is None:
        selected_analysis = select_best_hierarchical_analysis(hierarchical_analyses)
    else:
        selected_analysis = analyses_by_k.get(cut_k)
        if selected_analysis is None:
            raise ClusteringConfigError(
                f"Requested cut k={cut_k} is not available. Available values: {sorted(analyses_by_k)}"
            )

    labels = np.asarray(selected_analysis["labels"], dtype=int)
    numeric_features = list(feature_space["numeric_features"])
    numeric_normalized = np.asarray(feature_space["numeric_normalized"], dtype=float)
    categorical_features = list(feature_space["categorical_features"])
    categorical_matrix = np.asarray(feature_space["categorical_matrix"], dtype=object)

    output_dir = Path("cluster_graphs")
    saved_paths: list[str] = []

    for column_index, feature_name in enumerate(numeric_features):
        feature_values = numeric_normalized[:, column_index]
        output_path = output_dir / f"1D_density_{_sanitize_feature_name_for_path(feature_name)}.png"
        _plot_continuous_feature_density(
    ...
    for column_index, feature_name in enumerate(categorical_features):
        feature_values = categorical_matrix[:, column_index]
        output_path = output_dir / f"1D_density_{_sanitize_feature_name_for_path(feature_name)}.png"
        _plot_categorical_feature_density(
            feature_name=feature_name,
            categorical_values=feature_values,
            labels=labels,
            output_path=output_path,
        )
        saved_paths.append(str(output_path))
    ...
    print(
        "Saved 1D density visualisations for cut",
        int(selected_analysis["n_clusters_requested"]),
        "to",
        output_dir,
    )
...
globals()["1D_density_visualisation"] = one_d_density_visualisation
